  ...
  def self.perform_import(file)
    ...
    CSV.foreach(file) do |row|
      ...
      object_status << ObjectStatus.new(object_id: row[0],
                                        object_type: row[1],
                                        timestamp: row[2],
                                        object_changes: JSON.parse(row[3].to_s))


      # Import and Update the status for each NUM_MAX_IMPORT records
      if NUM_MAX_IMPORT == row_index || total_rows_count == $.
        broadcast_import_status("Importing the CSV file ... #{($..to_f * 100/total_rows_count).round(1)}% completed")
